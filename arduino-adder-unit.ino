...
const uint8_t numRegisterBits = 8;  // 4 bits in each operand
const int8_t minOperandNum = -8;  // -2^(4 - 1)
const uint8_t maxOperandNum = 7;  // 2^(4 - 1) - 1
...
void setup()
{
    ...
    Serial.print(minOperandNum);
    Serial.print(", ");
    delay(50);  // Prints incorrectly without small delay
    Serial.print(maxOperandNum);
    Serial.println("]:");
...
void numOperation(String strOperation)
{
    ...
    //     an operand (2 -+ 2)
    if (numPlusSigns > 1 || numMinusSigns > 3
        || numPlusSigns == 1 && (numMinusSigns == 3
            || numMinusSigns == 2 && (minusSignLocs[0] != 0
                || minusSignLocs[1] - 1 != plusSignLoc)
            || numMinusSigns == 1 && minusSignLocs[0] != 0
...
{
    if (num1 < minOperandNum || num1 > maxOperandNum
        || num2 < minOperandNum || num2 > maxOperandNum)
    {
    ...
    if (result < minOperandNum)
    {
        ...
        result &= maxOperandNum;
    }
    else if (result > maxOperandNum)
    {
        // If dealing with a 4-bit result, only the 3 least significant
        // bits represent the value.  The most significant bit is
